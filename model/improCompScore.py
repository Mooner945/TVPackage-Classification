#-*- coding:utf-8 -*-
# 根据偏好喜爱度模型计算节目评分,加入调整系数

import sys
import time
import pandas as pd

class improCompScore:
    ...
    def Compute(self, row, program, user):
        ...
        pro_set = set(userData[u'名称关键字'])
        user_set = set(proData[u'设备号'])
        Cij = float(len(data))
        Cj = len(proData)
        Ci = len(userData)
        Ni = len(pro_set)
        Mj = len(user_set)
        sum_timeij, sum_timej, sum_timei = 0, 0, 0
        # 计算每个节目观看时间与片长的比例的总和
        ...
        for item in userData.index:
            sum_timei += float(userData.ix[item, u'观看时间'])
